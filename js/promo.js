...
function _nlBenchSousMediane(){
  ...
  _renderNLResult('Familles sous médiane',sous.slice(0,30).map(f=>{
    ...
    return{
      ...
      diag:pct!==null&&pct<50?`<button class="diag-btn i-danger-bg c-danger text-[9px] py-0.5 px-1.5" data-fam="${escapeHtml(f.fam)}" onclick="openDiagnostic(this.dataset.fam,'reseau')">🔍</button>`:'',
    };
  }),[
...
function _nlRupturesTopClients({topN}){
  ...
  const rows=[...rupMap.values()].map(r=>({
    ...
    ca:formatEuro(r.ca),
    diag:r.fam?`<button class="diag-btn i-danger-bg c-danger text-[9px] py-0.5 px-1.5" data-fam="${escapeHtml(r.fam)}" onclick="openDiagnostic(this.dataset.fam,'reseau')">🔍</button>`:'',
