...
const fl=l=>q?l.filter(x=>matchQuery(q,x.code,x.lib)):l;const fM=fl(missed),fO=fl(over);
  ...
  if(_famSel){const _famSet=new Set(fM.map(x=>_S.articleFamille?.[x.code]||''));let _opts='<option value="">Toutes familles</option>';for(const f of[..._famSet].filter(Boolean).sort())_opts+=`<option value="${f}"${f===_famFilter?' selected':''}>${famLib(f)}</option>`;_famSel.innerHTML=_opts;_famSel.value=_famFilter;}
  const fMFiltered=_famFilter?fM.filter(m=>(_S.articleFamille?.[m.code]||'')===_famFilter):fM;
  const _MISSED_PAGE=10;const _TOP5=5;
  const _mSortCol=_S._missedSortCol||'';const _mSortDir=_S._missedSortDir||'desc';
  const fMSorted=_mSortCol?[...fMFiltered].sort((a,b)=>{const va=_mSortCol==='freq'?a.bassinFreq:_mSortCol==='agences'?a.sc:_mSortCol==='myCA'?a.myCA:a.myStock;const vb=_mSortCol==='freq'?b.bassinFreq:_mSortCol==='agences'?b.sc:_mSortCol==='myCA'?b.myCA:b.myStock;return _mSortDir==='desc'?vb-va:va-vb;}):fMFiltered;
  ...
  ['freq','agences','myCA','stock'].forEach(c=>{const el=document.getElementById('missedSort'+c.charAt(0).toUpperCase()+c.slice(1));if(el)el.textContent=c===_mSortCol?(_mSortDir==='desc'?'↓':'↑'):'';});
  // Detail tables
  ...
  if(!_mShowAll&&fMFiltered.length>_TOP5)p.push(`<tr><td colspan="6" class="text-center py-3"><button data-action="_reseauShowAll" data-section="missed" class="text-xs s-card border b-default rounded px-3 py-1.5 font-bold hover:s-hover t-secondary">Voir les ${fMFiltered.length} articles →</button></td></tr>`);
  else if(_mShowAll&&_totalMissedPages>1)p.push(`<tr><td colspan="6" class="text-center py-2"><div class="inline-flex items-center gap-2 text-xs"><button data-action="_reseauPage" data-section="missed" data-dir="-1" ${_curMissedPage===0?'disabled':''} class="px-2 py-1 s-card border b-default rounded hover:s-hover disabled:opacity-30 disabled:cursor-not-allowed">←</button><span class="t-secondary font-semibold">Page ${_curMissedPage+1} sur ${_totalMissedPages}</span><button data-action="_reseauPage" data-section="missed" data-dir="1" ${_curMissedPage>=_totalMissedPages-1?'disabled':''} class="px-2 py-1 s-card border b-default rounded hover:s-hover disabled:opacity-30 disabled:cursor-not-allowed">→</button></div></td></tr>`);
  rT('benchMissedTable',p.join('')||'<tr><td colspan="6" class="text-center py-4 t-disabled">🎉 Aucun article manquant</td></tr>');
