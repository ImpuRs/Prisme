...
const fl=l=>q?l.filter(x=>matchQuery(q,x.code,x.lib)):l;const fM=fl(missed),fO=fl(over),fU=fl(DataStore.benchLists.under||[]);
  ...
  function renderMesClients(){
    ...
    </div>`:`<div class="mb-5 p-4 s-card rounded-xl border text-[12px] t-secondary">🎯 <strong>Opportunités</strong> : ${_S.chalandiseReady?'Données insuffisantes pour calculer les opportunités.':'Chargez la zone de chalandise pour calculer.'}</div>`;
