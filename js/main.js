...
import { initRouter } from './router.js';
  ...
  function _renderOverviewL2(el,direction){
    ...
    metiersArr.forEach((m,mIdx)=>{
      ...
      html+=`<tr class="border-t border-rose-200 hover:i-danger-bg cursor-pointer font-semibold" onclick="_toggleOverviewL3('${dirEnc}','${mEnc}','${rowId}')">
  ...
  async function processData(){
    ...
    try{
      ...
      switchTab('action');btn.textContent='✅ '+elapsed+'s';btn.classList.replace('s-panel-inner','bg-green-600');
...
const fl=l=>q?l.filter(x=>(x.code+' '+x.lib).toLowerCase().includes(q)):l;const fM=fl(missed),fU=fl(under),fO=fl(over);
  ...
  function renderObservatoire(){
    ...
    if(diagEl&&kpis){
      ...
      const actHtml=diag.actions.map(a=>`<button onclick="_obsNav('${a.nav}')" class="text-[11px] font-semibold c-action underline hover:text-blue-900 bg-transparent border-none p-0 cursor-pointer">${a.label}</button>`).join('<span class="t-disabled mx-1">·</span>');
  ...
  function renderTable(pageOnly){
    ...
    for(const r of pd){
      ...
      const _medMinCell=showMed?(r.medMinReseau!=null?`<td class="px-2 py-2 text-center text-xs ${r.nouveauMin>2*r.medMinReseau?'c-caution i-caution-bg font-bold':r.nouveauMin>r.medMinReseau?'text-amber-600 font-semibold':'t-disabled'}" title="Méd. réseau MIN = ${Math.round(r.medMinReseau)}">${Math.round(r.medMinReseau)}</td>`:'<td class="px-2 py-2 text-center text-xs t-disabled">—</td>'):'';
      const _medMaxCell=showMed?(r.medMaxReseau!=null?`<td class="px-2 py-2 text-center text-xs ${r.nouveauMax>2*r.medMaxReseau?'c-caution i-caution-bg font-bold':r.nouveauMax>r.medMaxReseau?'text-amber-600 font-semibold':'t-disabled'}" title="Méd. réseau MAX = ${Math.round(r.medMaxReseau)}">${Math.round(r.medMaxReseau)}</td>`:'<td class="px-2 py-2 text-center text-xs t-disabled">—</td>'):'';
    ...
    if(v.cellMode){
      const metiersHtml=v.metiers.map((m,mIdx)=>{
        ...
        return`<div class="${mIdx>0?'mt-3 pt-3 border-t b-dark':''} s-panel-inner/50 rounded-lg p-3">
          ...
          ${(m.perdus>0||m.prospects>0)?`<p class="text-[11px] mb-1">${m.perdus>0?`<span class="c-danger">→ <strong>${m.perdus}</strong> perdu${m.perdus>1?'s':''} avec historique d'achat (reconquête)</span>`:''}${m.perdus>0&&m.prospects>0?' · ':''}${m.prospects>0?`<span class="text-orange-400">${m.prospects} prospect${m.prospects>1?'s':''} métier (conquête)</span>`:''}</p>`:''}
      ...
      const perdusTxt=[v.perdus>0?`<span class="c-danger">${v.perdus} perdu${v.perdus>1?'s':''} avec historique (reconquête)</span>`:'',totalProspects>0?`<span class="text-orange-400">${totalProspects} prospect${totalProspects>1?'s':''} métier (conquête)</span>`:''].filter(Boolean).join(' · ');
    ...
    return`<div class="mt-4 p-4 s-panel-inner border b-dark rounded-xl">
      <div class="flex items-center justify-between mb-3">
        ...
        <div class="flex items-center gap-2">
          <button id="_diagCopyBtn" onclick="_copyDiagPlan()" class="text-[10px] s-panel-inner hover:bg-slate-600 t-inverse py-1 px-3 rounded font-bold">📋 Copier</button>
          <button onclick="exportDiagnosticCSV('${famille.replace(/'/g,"\\'")}')" class="text-[10px] s-panel-inner hover:bg-slate-600 t-inverse py-1 px-3 rounded font-bold">📥 CSV</button>
        </div>
      </div>
