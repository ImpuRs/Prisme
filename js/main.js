...
import { _normFamGlobal, openDiagnostic, openDiagnosticMetier, closeDiagnostic, executeDiagAction, closeArticlePanel, openArticlePanel, renderDiagnosticPanel, _renderDiagnosticCellPanel, exportDiagnosticCSV } from './diagnostic.js';
  ...
  function _buildCockpitClient(){
    ...
    if(_S.opportuniteNette&&_S.opportuniteNette.length>0){
      ...
      const _opRows=_opTop.map(o=>{
        ...
        return`<div class="p-2 s-card rounded-lg border mb-1"><div class="flex items-center gap-2 flex-wrap"><span class="font-mono t-disabled text-[10px]">${o.cc}</span><span class="font-bold text-sm">${o.nom}</span>${_spcBadge(_opSpc)}<span class="text-[10px] t-tertiary">${o.metier}</span></div><div class="flex flex-wrap gap-1 mt-1">${fams}</div><div class="text-[10px] t-tertiary mt-1">Potentiel estimé : <strong class="c-action">${formatEuro(o.totalPotentiel)}</strong>/an · ${o.nbMissing} famille${o.nbMissing>1?'s':''} manquante${o.nbMissing>1?'s':''}</div></div>`;
      }).join('');
      _opNetBlock=`<details class="mb-4 s-card rounded-xl shadow-md border overflow-hidden"><summary class="p-4 cursor-pointer flex items-center justify-between"><h3 class="font-extrabold t-primary flex items-center gap-2">🎯 Opportunités nettes <span class="text-[10px] font-normal t-disabled">${_S.opportuniteNette.length} clients avec familles manquantes</span></h3><span class="acc-arrow t-disabled">▶</span></summary><div class="p-4 pt-0">${_opRows}</div></details>`;
    }
