...
<div class="periode-section">
  ...
  function renderTerritoireTab(){
    // ── Blocs Clients PDV (Top 5, Top PDV, Hors zone, Reconquête, Opportunités) ──
    {
      ...
      // ── Section 2 : Livrés sans PDV — tableau paginé 20/page ──
      const _livAll=_S.livraisonsSansPDV||[];
      const _LIV_PAGE=20;
      const livSansPDVHtml=(()=>{
        if(!_livAll.length)return _S.livraisonsReady?`<div class="mb-5 p-4 s-card rounded-xl border text-[12px] t-secondary">📦 <strong>Livrés sans PDV</strong> : Tous les clients livrés ont déjà acheté au comptoir.</div>`:'';
        const maxPage=Math.ceil(_livAll.length/_LIV_PAGE);
        const cur=Math.max(1,Math.min(_S._livSansPDVPage||1,maxPage));
        if(_S._livSansPDVPage!==cur)_S._livSansPDVPage=cur;
        const slice=_livAll.slice((cur-1)*_LIV_PAGE,cur*_LIV_PAGE);
        const rows=slice.map(r=>`<tr class="border-b b-light hover:s-hover cursor-pointer transition-colors" onclick="openClient360('${r.cc}','territoire')"><td class="py-1.5 px-2 font-bold text-[11px]">${r.nom}</td><td class="py-1.5 px-2 text-[11px] t-tertiary">${r.metier||'—'}</td><td class="py-1.5 px-2 text-right font-bold c-action text-[11px]">${formatEuro(r.caLivraison)}</td><td class="py-1.5 px-2 text-right text-[11px] t-tertiary">${r.nbBL}</td><td class="py-1.5 px-2 text-[11px] c-action">${r.commercial||'—'}</td></tr>`).join('');
        const prev=cur>1?`<button onclick="window._livSansPDVPage(-1)" class="text-[11px] font-semibold c-action hover:underline px-1 cursor-pointer">←</button>`:`<span class="text-[11px] t-disabled px-1">←</span>`;
        const next=cur<maxPage?`<button onclick="window._livSansPDVPage(1)" class="text-[11px] font-semibold c-action hover:underline px-1 cursor-pointer">→</button>`:`<span class="text-[11px] t-disabled px-1">→</span>`;
        const pager=`<div class="px-4 py-2 border-t b-default flex items-center justify-between"><span class="text-[10px] t-disabled">${_livAll.length} clients</span><div class="flex items-center gap-1">${prev}<span class="text-[11px] t-secondary">Page ${cur}/${maxPage}</span>${next}</div><span class="text-[10px] t-disabled">${(cur-1)*_LIV_PAGE+1}–${Math.min(cur*_LIV_PAGE,_livAll.length)}</span></div>`;
        return`<div class="mb-5 s-card rounded-xl border overflow-hidden"><div class="flex items-center gap-2 px-4 py-3 s-card-alt border-b"><h3 class="font-extrabold text-sm t-primary">📦 Livrés sans PDV <span class="text-[10px] font-normal t-disabled ml-1">${_livAll.length} clients</span></h3></div><div class="overflow-x-auto"><table class="min-w-full text-xs"><thead class="s-panel-inner t-inverse font-bold"><tr><th class="py-2 px-2 text-left">Nom client</th><th class="py-2 px-2 text-left">Métier</th><th class="py-2 px-2 text-right">CA livraison</th><th class="py-2 px-2 text-right">Nb BL</th><th class="py-2 px-2 text-left">Commercial</th></tr></thead><tbody>${rows}</tbody></table></div>${pager}</div>`;
      })();
      _setEl('terrReconquete', reconqHtml + livSansPDVHtml);

      // Opportunités nettes — tableau paginé
      _setEl('terrOpportunites',(()=>{const _OPP_PAGE=20;const _gc=_S._globalCanal||'';const _oppAll=(_S.opportuniteNette||[]).filter(o=>!_gc||(o.canalBreakdown&&(o.canalBreakdown[_gc]||0)>0));const _oppMaxP=Math.max(1,Math.ceil(_oppAll.length/_OPP_PAGE));const _oppCur=Math.max(1,Math.min(_S._oppNettePage||1,_oppMaxP));if(_S._oppNettePage!==_oppCur)_S._oppNettePage=_oppCur;const _oppSlice=_oppAll.slice((_oppCur-1)*_OPP_PAGE,_oppCur*_OPP_PAGE);const _oppPrev=_oppCur>1?`<button onclick="window._oppNettePage(-1)" class="text-[11px] font-semibold c-action hover:underline px-1 cursor-pointer">←</button>`:`<span class="text-[11px] t-disabled px-1">←</span>`;const _oppNext=_oppCur<_oppMaxP?`<button onclick="window._oppNettePage(1)" class="text-[11px] font-semibold c-action hover:underline px-1 cursor-pointer">→</button>`:`<span class="text-[11px] t-disabled px-1">→</span>`;const _oppPager=`<div class="px-4 py-2 border-t b-default flex items-center justify-between"><span class="text-[10px] t-disabled">${_oppAll.length} clients</span><div class="flex items-center gap-1">${_oppPrev}<span class="text-[11px] t-secondary">Page ${_oppCur}/${_oppMaxP}</span>${_oppNext}</div><span class="text-[10px] t-disabled">${(_oppCur-1)*_OPP_PAGE+1}–${Math.min(_oppCur*_OPP_PAGE,_oppAll.length)}</span></div>`;const _oppTotalCA=_oppAll.reduce((s,o)=>s+o.totalPotentiel,0);return _oppAll.length?`<div class="mb-5 s-card rounded-xl border overflow-hidden"><div class="flex items-center gap-2 px-4 py-3 s-card-alt border-b"><h3 class="font-extrabold text-sm t-primary">🎯 Opportunités nettes <span class="text-[10px] font-normal t-disabled ml-1">${_oppAll.length} clients · ${formatEuro(_oppTotalCA)} rapatriable</span></h3></div><div class="overflow-x-auto"><table class="w-full text-[11px] border-collapse"><thead><tr class="s-card-alt border-b"><th class="text-left px-3 py-2 font-semibold t-secondary">Client</th><th class="text-left px-3 py-2 font-semibold t-secondary">Métier</th><th class="text-left px-3 py-2 font-semibold t-secondary">Articles manquants</th><th class="text-right px-3 py-2 font-semibold t-secondary">CA potentiel</th><th class="text-left px-3 py-2 font-semibold t-secondary">Canal</th></tr></thead><tbody>${_oppSlice.map(o=>{const cp=Object.entries(o.canalBreakdown||{}).sort((a,b)=>b[1]-a[1])[0]?.[0]||'—';const arts=o.articlesManquants.slice(0,3).map(a=>`<span class="text-[9px] px-1 py-0.5 rounded i-info-bg c-action font-mono">${a.code}</span>`).join(' ')+(o.articlesManquants.length>3?` <span class="text-[9px] t-disabled">+${o.articlesManquants.length-3}</span>`:'');return`<tr class="border-b b-default hover:i-info-bg cursor-pointer transition-colors" onclick="openClient360('${o.cc}','clients')"><td class="px-3 py-2 font-semibold t-primary">${o.nom}</td><td class="px-3 py-2 t-secondary">${o.metier||'—'}</td><td class="px-3 py-2">${arts}</td><td class="px-3 py-2 text-right font-bold c-action">${formatEuro(o.totalPotentiel)}</td><td class="px-3 py-2 t-secondary text-[10px]">${cp}</td></tr>`;}).join('')}</tbody></table></div>${_oppPager}</div>`:`<div class="mb-5 p-4 s-card rounded-xl border text-[12px] t-secondary">🎯 <strong>Opportunités</strong> : Données insuffisantes pour calculer les opportunités.</div>`;})());
...
const fl=l=>q?l.filter(x=>matchQuery(q,x.code,x.lib)):l;const fM=fl(missed),fO=fl(over),fU=fl(DataStore.benchLists.under||[]);
  ...
  function renderMesClients(){
    ...
    // ── S2b: Livrés sans PDV — tableau paginé 20/page ────────────────
    const _livAllB=_S.livraisonsSansPDV||[];
    const _LIV_PAGE_B=20;
    const livSPDVHtml=(()=>{
      if(!_livAllB.length)return _S.livraisonsReady?`<div class="mb-5 p-4 s-card rounded-xl border text-[12px] t-secondary">📦 <strong>Livrés sans PDV</strong> : Tous les clients livrés ont déjà acheté au comptoir.</div>`:'';
      const maxPage=Math.ceil(_livAllB.length/_LIV_PAGE_B);
      const cur=Math.max(1,Math.min(_S._livSansPDVPage||1,maxPage));
      if(_S._livSansPDVPage!==cur)_S._livSansPDVPage=cur;
      const slice=_livAllB.slice((cur-1)*_LIV_PAGE_B,cur*_LIV_PAGE_B);
      const rows=slice.map(r=>`<tr class="border-b b-light hover:s-hover cursor-pointer transition-colors" onclick="openClient360('${r.cc}','clients')"><td class="py-1.5 px-2 font-bold text-[11px]">${r.nom}</td><td class="py-1.5 px-2 text-[11px] t-tertiary">${r.metier||'—'}</td><td class="py-1.5 px-2 text-right font-bold c-action text-[11px]">${formatEuro(r.caLivraison)}</td><td class="py-1.5 px-2 text-right text-[11px] t-tertiary">${r.nbBL}</td><td class="py-1.5 px-2 text-[11px] c-action">${r.commercial||'—'}</td></tr>`).join('');
      const prev=cur>1?`<button onclick="window._livSansPDVPage(-1)" class="text-[11px] font-semibold c-action hover:underline px-1 cursor-pointer">←</button>`:`<span class="text-[11px] t-disabled px-1">←</span>`;
      const next=cur<maxPage?`<button onclick="window._livSansPDVPage(1)" class="text-[11px] font-semibold c-action hover:underline px-1 cursor-pointer">→</button>`:`<span class="text-[11px] t-disabled px-1">→</span>`;
      const pager=`<div class="px-4 py-2 border-t b-default flex items-center justify-between"><span class="text-[10px] t-disabled">${_livAllB.length} clients</span><div class="flex items-center gap-1">${prev}<span class="text-[11px] t-secondary">Page ${cur}/${maxPage}</span>${next}</div><span class="text-[10px] t-disabled">${(cur-1)*_LIV_PAGE_B+1}–${Math.min(cur*_LIV_PAGE_B,_livAllB.length)}</span></div>`;
      return`<div class="mb-5 s-card rounded-xl border overflow-hidden"><div class="flex items-center gap-2 px-4 py-3 s-card-alt border-b"><h3 class="font-extrabold text-sm t-primary">📦 Livrés sans PDV <span class="text-[10px] font-normal t-disabled ml-1">${_livAllB.length} clients</span></h3></div><div class="overflow-x-auto"><table class="min-w-full text-xs"><thead class="s-panel-inner t-inverse font-bold"><tr><th class="py-2 px-2 text-left">Nom client</th><th class="py-2 px-2 text-left">Métier</th><th class="py-2 px-2 text-right">CA livraison</th><th class="py-2 px-2 text-right">Nb BL</th><th class="py-2 px-2 text-left">Commercial</th></tr></thead><tbody>${rows}</tbody></table></div>${pager}</div>`;
    })();

    // ── S3: Opportunités nettes — tableau paginé ──────────────────────────────
