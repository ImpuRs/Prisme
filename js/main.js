...
import { showToast, updateProgress, updatePipeline, showLoading, hideLoading, showTerritoireLoading, updateTerrProgress, onFileSelected, collapseImportZone, expandImportZone, switchTab, openFilterDrawer, closeFilterDrawer, populateSelect, getFilteredData, renderAll, onFilterChange, debouncedRender, resetFilters, filterByAge, clearAgeFilter, updateActiveAgeIndicator, filterByAbcFmr, showCockpitInTable, clearCockpitFilter, _toggleNouveautesFilter, updatePeriodAlert, renderInsightsBanner, openReporting, sortBy, changePage, openCmdPalette, _cmdExec, _cmdMoveSelection, _cmdRender, _cmdBuildResults, closeReporting, copyReportText, clearSavedKPI, exportKPIhistory, importKPIhistory, downloadCSV, renderCockpitBriefing, renderDecisionQueue, dqFocus, clipERP, wrapGlossaryTerms, initTheme, cycleTheme, exportCockpitResume } from './ui.js';
...
import { openDiagnostic, openDiagnosticMetier, closeDiagnostic, executeDiagAction, closeArticlePanel, openArticlePanel, renderDiagnosticPanel, _renderDiagnosticCellPanel, exportDiagnosticCSV, _diagV3FilterCategory, toggleReconquestFilter, openClient360, _c360SwitchTab, _c360CopyResume } from './diagnostic.js';
  ...
  function _buildDegradedCockpit(){
    ...
    if(topClients.length){
      ...
      if(hasChal){
        html+=`<details class="s-card rounded-xl shadow-md border mb-3 overflow-hidden"><summary class="flex items-center gap-2 p-3 border-b cursor-pointer list-none flex-wrap"><span>⭐</span><h4 class="font-extrabold text-sm flex-1">Top clients PDV <span class="text-[10px] font-normal t-disabled">${topClients.length} client${topClients.length>1?'s':''}</span></h4>${_toggle}<span class="text-[10px] t-disabled">▼</span></summary>${_topTable}</details>`;
      } else {
  ...
  async function processDataFromRaw(dataC,dataS,opts={}){
    ...
    try{
      ...
      if(cc2&&code&&(!_S.selectedMyStore||sk===_S.selectedMyStore)&&(qteP>0||qteE>0)){if(!_S.ventesClientArticle.has(cc2))_S.ventesClientArticle.set(cc2,new Map());const artMap=_S.ventesClientArticle.get(cc2);if(!artMap.has(code))artMap.set(code,{sumPrelevee:0,sumCAPrelevee:0,sumCA:0,countBL:0});const e=artMap.get(code);if(qteP>0){e.sumPrelevee+=qteP;e.sumCAPrelevee+=caP;}e.sumCA+=caP+caE;e.countBL++;}
...
const fl=l=>q?l.filter(x=>matchQuery(q,x.code,x.lib)):l;const fM=fl(missed),fU=fl(under),fO=fl(over);
  ...
  function renderObsArticleSearch(){
    ...
    if(!rows.length){
      ...
      for(const[code,d]of Object.entries(agenceData)){
        ...
        const myStock=_S.finalData.find(r=>r.code===code)?.stockActuel??null;
  ...
  function renderDashboardAndCockpit(){
    ...
    // P1/P3: populate #rupturesSummary with critiques count
    {const critiques=lstR.filter(r=>r.prioScore>=70).length;const summaryEl=document.getElementById('rupturesSummary');if(summaryEl)summaryEl.textContent=critiques>0?`dont ${critiques} 🔥 Critiques`:'';};
    ...
    document.getElementById('actionRuptures').innerHTML=p.join('')||'<tr><td colspan="8" class="text-center py-4 t-disabled text-xs">🎉 Aucune rupture</td></tr>';
    const ruptTotEl=document.getElementById('actionRupturesTotal');
    {const caPerduFooter=hasMulti?`💸 CA perdu estimé : ${formatEuro(totalCAPerdu)} (vs médiane réseau)`:totalCAPerdu>=100?`💸 CA historique des articles en rupture : ${formatEuro(totalCAPerdu)}`:`💸 CA perdu non estimable — historique insuffisant`;if(ruptTotEl)ruptTotEl.innerHTML=lstR.length>0?`<tr><td colspan="8" class="py-2 px-2 text-right text-xs font-extrabold c-danger border-t b-light">${caPerduFooter}</td></tr><tr><td colspan="8" class="py-1 px-2 text-right"><button onclick="event.stopPropagation();_copyAllCodesDirect(this,this.dataset.codes)" data-codes="${lstR.slice(0,50).map(r=>r.code).join(',')}" class="text-[10px] t-disabled hover:t-primary s-card border b-default rounded px-1.5 py-0.5">📋 Copier ${Math.min(lstR.length,50)} codes</button></td></tr>`:'';};
