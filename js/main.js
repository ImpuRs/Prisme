...
import { _normFamGlobal, openDiagnostic, openDiagnosticMetier, closeDiagnostic, executeDiagAction, closeArticlePanel, openArticlePanel, renderDiagnosticPanel, _renderDiagnosticCellPanel, exportDiagnosticCSV, _diagV3FilterCategory, toggleReconquestFilter, openClient360, _c360SwitchTab, _c360CopyResume } from './diagnostic.js';
  ...
  async function processDataFromRaw(dataC,dataS,opts={}){
    ...
    try{
      ...
      if(hasMulti){const sel=document.getElementById('selectMyStore');sel.innerHTML='<option value="">—</option>';[..._S.storesIntersection].sort().forEach(s=>{const o=document.createElement('option');o.value=s;o.textContent=s;sel.appendChild(o);});document.getElementById('storeSelector').classList.remove('hidden');document.getElementById('storeInfo').innerHTML=`✅ ${_S.storesIntersection.size} mag.`;const _saved=localStorage.getItem('prisme_selectedStore');if(_saved&&_S.storesIntersection.has(_saved)){_S.selectedMyStore=_saved;sel.value=_saved;btn.disabled=false;}else if(_S.storesIntersection.size===1){_S.selectedMyStore=[..._S.storesIntersection][0];sel.value=_S.selectedMyStore;localStorage.setItem('prisme_selectedStore',_S.selectedMyStore);btn.disabled=false;}else{_S.selectedMyStore='';sel.value='';btn.disabled=false;hideLoading();showToast('⚠️ Sélectionnez votre agence dans "Mon Magasin" puis cliquez Recalculer','warning');throw new Error('NO_STORE_SELECTED');}}
