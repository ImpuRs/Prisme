    ...
    if(_refilterDataC?.rows?.length){
      ...
      (async()=>{
        ...
        try{
          ...
          computeClientCrossing();_computeClientDominantUnivers();
          renderCanalAgence();renderCurrentTab();
          if(_S.storesIntersection.size>1)launchReseauWorker().then(()=>{renderNomadesMissedArts();}).catch(err=>console.warn('[refilter] Réseau worker:',err));
...
<div class="periode-section">
  ...
  function generateRegionReportText(){
    ...
    const L=[];
    L.push(`INSTRUCTIONS`);
    L.push(`Tu es un expert en analyse commerciale ET en rédaction percutante, dans le rôle d'un chef d'agence en distribution B2B quincaillerie (réseau Legallais).`);
    L.push(`Rédige ton reporting mensuel pour ta direction régionale, à la première personne.`);
    L.push(`Ce texte sera collé dans une cellule Excel — compact, scannable, impactant.`);
    L.push(`Règles d'écriture :`);
    L.push(`  - Chaque phrase doit contenir un chiffre ET un jugement (bien/mal/alerte/levier)`);
    L.push(`  - Commence chaque section par le fait le plus marquant, pas par le CA`);
    L.push(`  - Utilise des contrastes forts : "X mais Y", "malgré X, Y", "+201% ici, -51% là"`);
    L.push(`  - Mets en avant ce qui surprend, pas ce qui est attendu`);
    L.push(`  - TOUTES les données doivent apparaître — rien ne doit être ignoré`);
    L.push(`Structure :`);
    L.push(`  1. ACCROCHE — 1 phrase choc qui résume le mois (le fait le plus fort)`);
    L.push(`  2. PERFORMANCE — CA, rang, marge, canaux. Ce qui va, ce qui coince.`);
    L.push(`  3. STOCK — dispo, ruptures, dormants. Le bon et le mauvais.`);
    ...
    L.push(`  6. MES 3 ACTIONS — engagements datés, chiffrés, mesurables`);
    L.push(`Ton : un chef d'agence qui sait où il en est, qui assume et qui agit. Direct, nerveux, zéro langue de bois.`);
    L.push('');
    L.push(`═══════════════════════════════════════════════════`);
    L.push(`DONNÉES AGENCE ${agence} — ${periodLabel}`);
    if(filtersLabel)L.push(`FILTRES ACTIFS : ${filtersLabel}`);
  ...
  async function _postParseMain(opts) {
    ...
    try {
      ...
      launchClientWorker().then(async()=>{
        if(_S.chalandiseReady&&DataStore.ventesClientArticle.size>0){computeOpportuniteNette();computeOmniScores();computeFamillesHors();buildClientStore();renderTabBadges();updateLaboTiles();showToast('📊 Agrégats clients calculés','success');}
...
<div class="hero-layout">
  ...
  function renderTable(pageOnly){
    ...
    p.push(`<tr class="border-b hover:i-info-bg ${bg} cursor-pointer"${isUncalib?' style="opacity:0.48"':''}
      ...
      <td class="px-2 py-2 text-center font-extrabold c-action text-xs">${r.nouveauMin}</td>
