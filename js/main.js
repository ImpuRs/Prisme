...
import { initRouter } from './router.js';
  ...
  function _buildDeptFilter(){
    ...
    container.innerHTML=depts.map(([dept,cnt])=>{
      ...
      return`<label class="flex items-center gap-1.5 text-[10px] py-0.5 px-1 rounded cursor-pointer hover:bg-rose-50"><input type="checkbox" ${sel?'checked':''} onchange="_toggleDept('${dept}')" class="rounded accent-rose-600"><span class="font-semibold">${dept}</span><span class="t-disabled">(${cnt})</span></label>`;
    }).join('');
  ...
  function _resetChalandiseFilters(){_S._selectedDepts=new Set();_S._selectedClassifs=new Set();_S._selectedStatuts=new Set();_S._selectedActivitesPDV=new Set();_S._selectedCommercial='';_S._selectedMetier='';_S._filterStrategiqueOnly=false;_S._includePerdu24m=false;const btn=document.getElementById('btnStrategiqueOnly');if(btn){btn.classList.remove('bg-amber-500','text-white');btn.classList.add('bg-gray-200','t-secondary');}const cb=document.querySelector('#togglePerdu24m input');if(cb)cb.checked=false;const comSel=document.getElementById('terrCommercialFilter');if(comSel)comSel.value='';const metSel=document.getElementById('terrMetierFilter');if(metSel)metSel.value='';_buildDeptFilter();_buildChalandiseOverview();}
  ...
  function _toggleStrategiqueFilter(){_S._filterStrategiqueOnly=!_S._filterStrategiqueOnly;const btn=document.getElementById('btnStrategiqueOnly');if(btn){btn.classList.toggle('bg-amber-500',_S._filterStrategiqueOnly);btn.classList.toggle('text-white',_S._filterStrategiqueOnly);btn.classList.toggle('bg-gray-200',!_S._filterStrategiqueOnly);btn.classList.toggle('t-secondary',!_S._filterStrategiqueOnly);}if(_S._filterStrategiqueOnly&&_S._selectedMetier&&!_isMetierStrategique(_S._selectedMetier)){_S._selectedMetier='';const mi=document.getElementById('terrMetierFilter');if(mi)mi.value='';}_buildChalandiseOverview();}
  ...
  function _buildChalandiseOverview(){
    ...
    if(sumClients)sumClients.innerHTML=filterActive?`<span class="text-rose-600">${filteredClients.toLocaleString('fr-FR')}</span><span class="text-sm font-semibold t-disabled"> / ${totalClients.toLocaleString('fr-FR')}</span>`:`${filteredClients.toLocaleString('fr-FR')}`;
    ...
    dirsArr.forEach((d,idx)=>{
      ...
      html+=`<tr class="border-b text-[11px] hover:s-card-alt cursor-pointer font-semibold" onclick="_toggleOverviewL2('${dirEnc}',${idx})">
        ...
        <td class="py-1.5 px-2 text-center ${d.actifsLeg>0?'text-green-700 font-bold':'t-disabled'}">${d.actifsLeg||'—'}</td>
        <td class="py-1.5 px-2 text-center ${d.actifsPDV>0?'text-emerald-600 font-bold':'t-disabled'}">${d.actifsPDV||'—'}</td>
        <td class="py-1.5 px-2 text-center ${d.prospects>0?'c-action':'t-disabled'}">${d.prospects||'—'}</td>
        <td class="py-1.5 px-2 text-center ${d.perdus12_24>0?'text-orange-600 font-bold':'t-disabled'}">${d.perdus12_24||'—'}</td>
        <td class="py-1.5 px-2 text-center ${d.inactifs>0?'t-secondary':'t-disabled'}">${d.inactifs||'—'}</td>
        <td class="py-1.5 px-2"><div class="flex items-center gap-1"><div class="flex-1 bg-gray-200 rounded-full h-1.5"><div class="cap-bar bg-blue-400" style="width:${pctL}%"></div></div><span class="text-[10px] font-bold w-8 text-right c-action">${pctL}%</span></div></td>
      ...
      <tr id="overviewL2-${idx}" style="display:none"><td colspan="${colSpan}" class="p-0 bg-rose-50"><div id="overviewL2Inner-${idx}" class="text-xs t-disabled px-4 py-2">Chargement…</div></td></tr>`;
    });
  ...
  function _renderOverviewL2(el,direction){
    ...
    metiersArr.forEach((m,mIdx)=>{
      ...
      html+=`<tr class="border-t border-rose-200 hover:bg-rose-50 cursor-pointer font-semibold" onclick="_toggleOverviewL3('${dirEnc}','${mEnc}','${rowId}')">
        <td class="py-1.5 px-2">${m.metier}${_isMetierStrategique(m.metier)?' <span class="text-amber-500 text-[10px]" title="Métier stratégique Legallais">⭐</span>':''} <span id="${rowId}-arrow" class="t-disabled text-[9px]">▼</span></td>
        <td class="py-1.5 px-2 text-center font-bold">${m.total}</td>
        <td class="py-1.5 px-2 text-center ${m.actifsLeg>0?'text-green-700 font-bold':'t-disabled'}">${m.actifsLeg||'—'}</td>
        <td class="py-1.5 px-2 text-center ${m.actifsPDV>0?'text-emerald-600 font-bold':'t-disabled'}">${m.actifsPDV||'—'}</td>
        <td class="py-1.5 px-2 text-center ${m.prospects>0?'c-action':'t-disabled'}">${m.prospects||'—'}</td>
        <td class="py-1.5 px-2 text-center ${m.perdus12_24>0?'text-orange-600 font-bold':'t-disabled'}">${m.perdus12_24||'—'}</td>
        <td class="py-1.5 px-2 text-center ${m.inactifs>0?'t-secondary':'t-disabled'}">${m.inactifs||'—'}</td>
        <td class="py-1.5 px-2"><div class="flex items-center gap-1"><div class="flex-1 bg-gray-200 rounded-full h-1.5"><div class="cap-bar bg-blue-400" style="width:${pctL}%"></div></div><span class="text-[10px] w-7 text-right c-action">${pctL}%</span></div></td>
      ...
      <tr id="${rowId}" style="display:none"><td colspan="10" class="p-0 bg-violet-50"><div id="${rowId}-inner" class="text-xs t-disabled px-4 py-2">Chargement…</div></td></tr>`;
    });
  ...
  function _renderOverviewL3(el,direction,metier){
    ...
    sectsArr.forEach((s,sIdx)=>{
      ...
      html+=`<tr class="border-t border-violet-200 hover:bg-violet-50 cursor-pointer" onclick="_toggleOverviewL4('${dirEnc}','${mEnc}','${sEnc}','${rowId}')">
        ...
        <td class="py-1.5 px-2 text-center ${s.actifsLeg>0?'text-green-700 font-bold':'t-disabled'}">${s.actifsLeg||'—'}</td>
        <td class="py-1.5 px-2 text-center ${s.actifsPDV>0?'text-emerald-600 font-bold':'t-disabled'}">${s.actifsPDV||'—'}</td>
        <td class="py-1.5 px-2 text-center ${s.prospects>0?'c-action':'t-disabled'}">${s.prospects||'—'}</td>
        <td class="py-1.5 px-2 text-center ${s.perdus12_24>0?'text-orange-600 font-bold':'t-disabled'}">${s.perdus12_24||'—'}</td>
  ...
  function _renderOverviewL4(el,direction,metier,secteur,limit){
    ...
    for(const c of show){
      ...
      const pdvBg=globActif&&!c._pdvActif?'i-caution-bg':perdu?'bg-rose-50':'';
  ...
  function buildPeriodFilter(){
    ...
    function pill(label,startTs,endTs2){
      ...
      const cls=active?'bg-blue-700 text-white border-blue-700':'bg-white t-secondary border-gray-300 hover:border-blue-400 hover:c-action';
  ...
  function _buildDegradedCockpit(){
    ...
    const banner=`<div class="mb-3 p-3 i-caution-bg border border-amber-200 rounded-lg text-xs text-amber-800">💡 <strong>Chargez la Zone de Chalandise</strong> pour débloquer l'analyse métier, la captation et les prospects.</div>`;
    let html=banner;
    if(silencieux.length){
      const rows=silencieux.slice(0,20).map(c=>{const cls=c.d>90?'c-danger':c.d>60?'text-orange-500':'text-yellow-600';return`<tr class="border-t b-light"><td class="py-1 px-2 font-mono text-[10px] t-disabled">${c.cc}</td><td class="py-1 px-2 text-[11px] font-semibold">${c.nom}${_unikLink(c.cc)}</td><td class="py-1 px-2 text-right font-bold c-ok text-[11px]">${formatEuro(c.ca)}</td><td class="py-1 px-2 text-center font-bold text-[11px] ${cls}">${c.d}j</td></tr>`;}).join('');
    ...
    if(topClients.length){
      const rows=topClients.slice(0,10).map((c,i)=>`<tr class="border-t b-light"><td class="py-1 px-2 text-[10px] t-disabled font-bold">#${i+1}</td><td class="py-1 px-2 font-mono text-[10px] t-disabled">${c.cc}</td><td class="py-1 px-2 text-[11px] font-semibold">${c.nom}${_unikLink(c.cc)}${silSet.has(c.cc)?' <span class="text-[9px] bg-rose-100 text-rose-600 px-1 rounded-full">silencieux</span>':''}</td><td class="py-1 px-2 text-right font-bold c-ok text-[11px]">${formatEuro(c.ca)}</td><td class="py-1 px-2 text-center text-[10px] t-tertiary">${c.nbArts}</td></tr>`).join('');
      html+=`<div class="bg-white rounded-xl shadow-md border mb-3 overflow-hidden"><div class="flex items-center gap-2 p-3 border-b"><span>⭐</span><h4 class="font-extrabold text-sm flex-1">Top clients PDV <span class="text-[10px] font-normal t-disabled">${topClients.length} client${topClients.length>1?'s':''}</span></h4></div><div class="overflow-x-auto"><table class="min-w-full text-xs"><thead class="s-card-alt t-secondary font-bold text-[10px]"><tr><th class="py-1.5 px-2 text-center">#</th><th class="py-1.5 px-2 text-left">Code</th><th class="py-1.5 px-2 text-left">Nom</th><th class="py-1.5 px-2 text-right">CA Comptoir</th><th class="py-1.5 px-2 text-center">Réf</th></tr></thead><tbody>${rows}</tbody></table></div></div>`;
    }
  ...
  function _buildCockpitClient(){
    ...
    if(_S._selectedCrossStatus==='fidele'){
      if(!_S.crossingStats||!_S.crossingStats.fideles.size){el.innerHTML='<div class="bg-white rounded-xl shadow-md border p-5 t-disabled text-sm">Aucun fidèle hors zone identifié.</div>';return;}
    ...
    function _silColor(c){return c._daysSince>90?'c-danger':c._daysSince>60?'text-orange-600':'text-yellow-700';}
    ...
    function _fullTable(clients,sortField,listId){
      ...
      for(const c of clients){const caVal=usePDV?c.caPDVN:c.ca2025;const caColor=usePDV?(caVal>0?'c-ok':'t-disabled'):(caVal>0?'c-caution':'t-disabled');t+=`<tr class="border-t b-default hover:bg-white/50 cursor-pointer" onclick="_toggleClientArticles(this,'${c.code}')"><td class="py-1 px-2"><span class="font-mono t-disabled text-[10px]">${c.code}</span>${_crossBadge(c.code)} <span class="font-semibold">${c.nom}</span>${_unikLink(c.code)}${c._strat?' <span class="text-amber-500 text-[10px]" title="Métier stratégique">⭐</span>':''}${_clientStatusBadge(c.code,c)}</td><td class="py-1 px-2 text-[10px] t-tertiary">${c.commercial||'—'}</td><td class="py-1 px-2 text-center text-[10px]">${_classifShort(c.classification)}</td><td class="py-1 px-2 text-right font-bold ${caColor}">${caVal>0?formatEuro(caVal):'—'}</td><td class="py-1 px-2 text-[10px] t-tertiary">${c.ville||'—'}</td></tr>`;}
      t+=`</tbody></table></div>`;
      return t;
    }
    ...
    const banner=`<div class="mb-3 px-4 py-2 s-card-alt border rounded-lg text-[11px] font-semibold t-primary">Sur votre sélection : ${silBit}<span class="c-danger">🔴 ${urgences.length} à capter</span> · <span class="text-orange-500">🟠 ${developper.length} à développer</span> · <span class="c-ok">🟢 ${fideliser.length} à fidéliser</span></div>`;
    el.innerHTML=searchResultsHtml+`<div class="bg-white rounded-xl shadow-md border overflow-hidden"><div class="p-4 border-b bg-gradient-to-r from-rose-50 via-red-50 via-orange-50 to-green-50"><div class="flex items-center gap-2 flex-wrap"><h3 class="font-extrabold t-primary flex-1">👥 Cockpit Client</h3><button onclick="exportCockpitCSVAll()" class="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 py-1 px-2 rounded font-bold border">📥 Exporter tout</button><button onclick="exportExclusionsJSON()" class="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 py-1 px-2 rounded font-bold border">📤 Exclusions</button><button onclick="document.getElementById('importExclusionsInput').click()" class="text-[10px] bg-slate-100 hover:bg-slate-200 text-slate-700 py-1 px-2 rounded font-bold border">📥 Importer</button></div><p class="text-[10px] t-tertiary mt-0.5">Actions prioritaires basées sur la zone de chalandise${hasTerr?' et le territoire':''}. <span class="t-disabled">CA Legallais = CA global tous canaux · CA Comptoir Zone = achats dans votre PDV (source chalandise)</span></p></div><div class="p-4">${banner}<div class="grid grid-cols-1 gap-4">${renderBlock('ALERTE — Clients silencieux','Clients réguliers de votre agence sans commande depuis plus de 30 jours · {total} clients','🚨','bg-rose-50','border-rose-500','hover:bg-rose-50',_silColor,silencieux,'caPDVN',_silRaison,'cockpit-sil-full')}${renderBlock('À CAPTER — Actifs Legallais hors PDV','Clients qui achètent chez Legallais mais ne viennent pas en agence · {total} clients','🔴','i-danger-bg','border-red-500','hover:i-danger-bg','c-danger',urgences,'ca2025',_urgRaison,'cockpit-urg-full')}${renderBlock('À DÉVELOPPER — Top 10 priorités','Triés par potentiel · {total} clients dans cette catégorie','🟠','i-caution-bg','border-orange-500','hover:i-caution-bg','text-orange-700',developper,'ca2025',_devRaison,'cockpit-dev-full')}${renderBlock('À FIDÉLISER — Top 10 bons clients','Triés par CA Comptoir · {total} clients dans cette catégorie','🟢','i-ok-bg','border-green-500','hover:i-ok-bg','text-green-700',fideliser,'caPDVN',_fidRaison,'cockpit-fid-full')}</div></div></div>`;
  }
  function _setCrossFilter(status){
    ...
    for(const[id,val] of Object.entries(map)){
      ...
      btn.classList.toggle('bg-white',!active);btn.classList.toggle('t-primary',!active);btn.classList.toggle('border-gray-300',!active);
    }
  ...
  function _buildTerrDirStatusRows(arts,stockMap,isStd){
    ...
    for(const a of arts){
      ...
      const speTag=!isStd(a.code)?'<span class="ml-1 text-[9px] bg-gray-200 t-tertiary font-bold px-1 rounded">SPÉ</span>':'';
  ...
  function renderTerrContrib(){
    ...
    for(const r of rows){
      ...
      </tr><tr id="${rowId}" class="contrib-dir-detail"><td colspan="5" class="p-0 bg-violet-50 border-b border-violet-100"><div id="${rowId}-inner" class="p-4 text-xs t-disabled">Chargement…</div></td></tr>`;
    }
  ...
  function _buildClientRows(clients){
    ...
    for(const c of clients){
      ...
      </tr><tr id="${cliRowId}" class="contrib-client-detail"><td colspan="5" class="p-0 bg-white border-t b-light"><div id="${cliRowId}-inner" class="p-3 text-xs t-disabled">Chargement…</div></td></tr>`;
    }
    return html;
  }
  ...
  function renderContribArticles(rowId,clientCode){
    ...
    for(const a of arts.slice(0,LIMIT)){
      ...
      const speTag=a.isSpecial?'<span class="ml-1 text-[9px] bg-gray-200 t-tertiary font-bold px-1 rounded">SPÉ</span>':'';
  ...
  function _togglePromoClientArts(row,cc){
    ...
    const trs=allRows.map(([code,d])=>{
      ...
      return`<tr class="border-t border-emerald-100"><td class="py-0.5 px-2 font-mono t-disabled text-[9px]">${code}</td><td class="py-0.5 px-2 t-primary">${lib}</td><td class="py-0.5 px-2 text-center t-tertiary">${d.countBL||'—'}</td><td class="py-0.5 px-2 text-right font-bold c-ok">${ca>0?formatEuro(ca):'—'}</td><td class="py-0.5 px-2 text-center t-disabled">${fmtD(lastDate)}</td></tr>`;
    }).join('');
    const tr=document.createElement('tr');tr.className='promo-art-panel';
    tr.innerHTML=`<td colspan="6" class="py-2 px-4 i-ok-bg border-b border-emerald-100">${note}<table class="min-w-full text-[10px]"><thead class="bg-emerald-100 text-emerald-800 font-bold"><tr><th class="py-0.5 px-2 text-left">Code</th><th class="py-0.5 px-2 text-left">Libellé</th><th class="py-0.5 px-2 text-center">Qté</th><th class="py-0.5 px-2 text-right">CA</th><th class="py-0.5 px-2 text-center">Dernier achat</th></tr></thead><tbody>${trs}</tbody></table></td>`;
    row.after(tr);
  }
  ...
  function _renderPromoImportResults(){
    ...
    if(sumEl)sumEl.innerHTML=`<strong>${r.promoCodes.size}</strong> articles opération · <span class="c-ok">${sold} vendus (${formatEuro(totalCA)} CA Comptoir)</span> · <span class="c-danger">${unsold} non vendus</span> · <span class="text-orange-600">${retarget} clients à relancer</span>${r.opName?` · <em>${r.opName}</em>`:''}`;
    // Section D
    ...
    document.getElementById('promoImportTableF').innerHTML=r.sectionF.slice(0,200).map(x=>{
      ...
      return`<tr class="border-t border-orange-50 hover:i-caution-bg"><td class="py-1 px-2 font-mono t-disabled">${x.cc}</td><td class="py-1 px-2 font-semibold">${x.nom}${statutBadge}</td><td class="py-1 px-2 t-tertiary">${x.metier||'—'}</td><td class="py-1 px-2 text-right font-bold text-orange-700">${x.famCA>0?formatEuro(x.famCA):'—'}</td><td class="py-1 px-2 t-tertiary text-[10px]">${x.raison}</td><td class="py-1 px-2 t-tertiary">${x.commercial||'—'}</td></tr>`;
...
const fl=l=>q?l.filter(x=>(x.code+' '+x.lib).toLowerCase().includes(q)):l;const fM=fl(missed),fU=fl(under),fO=fl(over);
    ...
    p=[];const maxF=Math.max(...Object.values(storePerf).map(s=>s.freq),1);sorted.forEach(([store,data],idx)=>{const isMe=store===_S.selectedMyStore,bw=(data.freq/maxF*100).toFixed(0);const servTri=data.serv>=80?'<span class="text-emerald-600" aria-label="au-dessus">▲</span>':data.serv>=60?'<span class="text-amber-500" aria-label="neutre">—</span>':'<span class="c-danger" aria-label="en-dessous">▼</span>';const servTxt=servTri+' '+data.serv+'%';const servColor=data.serv>=80?'c-ok':data.serv>=60?'c-caution':'c-danger';const tmTxt=data.txMarge>0?data.txMarge.toFixed(2)+'%':'—';const tmColor=data.txMarge>0?(data.txMarge>=35?'c-ok':data.txMarge>=25?'c-caution':'c-danger'):'t-disabled';const cz=showClientsZone?`<td class="py-2 px-2 text-center text-xs font-bold ${data.clientsZone>0?'text-rose-700':'t-disabled'}">${data.clientsZone>0?data.clientsZone:'—'}</td>`:'';p.push(`<tr class="border-b ${isMe?'bg-cyan-50 font-bold':'hover:s-card-alt'}"><td class="py-2 px-2"><span class="${isMe?'store-tag store-mine':'store-tag store-other'}">${isMe?'⭐':''}${store}</span></td><td class="py-2 px-2 text-center">${data.ref}</td><td class="py-2 px-2 text-center ${isMe?'text-cyan-700 font-extrabold':'font-bold'}">${data.freq.toLocaleString('fr')}</td><td class="py-2 px-2 text-center ${servColor} text-[10px] font-bold">${servTxt}</td><td class="py-2 px-2 text-center text-[11px] font-bold ${tmColor}">${tmTxt}</td>${cz}<td class="py-2 px-2 text-right"><div class="flex items-center gap-1 justify-end"><div class="w-16 bg-gray-200 rounded-full h-2"><div class="perf-bar ${isMe?'bg-cyan-500':'bg-gray-400'} rounded-full" style="width:${bw}%"></div></div><span class="text-[10px] font-bold ${isMe?'text-cyan-700':''}">#${idx+1}/${totalStores}</span></div></td></tr>`);});
  ...
  function generateNetworkDiagnostic(caE,refE,freqE){
    ...
    return{icon:'📋',title:'Analyse mixte',border:'border-gray-300',bg:'s-card-alt',
  ...
  function renderObservatoire(){
    ...
    const cardsHtml=kpiDefs.map(r=>{
      ...
      const ecartColor=pct>=0?'c-ok font-extrabold':pct>=-10?'text-yellow-700 font-bold':pct>=-30?'c-caution font-bold':'c-danger font-extrabold';
    ...
    if(diagEl&&kpis){
      ...
      const actHtml=diag.actions.map(a=>`<button onclick="_obsNav('${a.nav}')" class="text-[11px] font-semibold c-action underline hover:text-blue-900 bg-transparent border-none p-0 cursor-pointer">${a.label}</button>`).join('<span class="text-gray-300 mx-1">·</span>');
    ...
    const loseRows=loseFiltered.map((f,i)=>{
      ...
      const copyBtn=artsToRef.length?`<button onclick="event.stopPropagation();copyObsArticleList('obsLose_${i}_arts')" class="text-[9px] bg-white border border-red-200 c-danger px-1.5 py-0.5 rounded hover:i-danger-bg font-bold">📋 Copier liste 1</button>`:'';
    ...
    const winRows=winFiltered.map((f,i)=>{
      ...
      const exclHtml=excl.length?`<div class="mb-3 px-3"><p class="text-[10px] font-bold c-ok mb-1">🏆 ${excl.length} article${excl.length>1?'s':''} exclusifs — vous les vendez, pas ${isMedian?'la médiane':obsLabel} :</p><table class="min-w-full text-[10px]"><thead class="t-secondary font-bold"><tr><th class="py-1 px-2 text-left">Code</th><th class="py-1 px-2 text-left">Libellé</th><th class="py-1 px-2 text-center">Fréq</th><th class="py-1 px-2 text-right">CA</th></tr></thead><tbody>${excl.slice(0,20).map(a=>`<tr class="border-b border-emerald-100"><td class="py-0.5 px-2 font-mono t-tertiary">${a.code}</td><td class="py-0.5 px-2">${a.lib}</td><td class="py-0.5 px-2 text-center font-bold">${a.freq}</td><td class="py-0.5 px-2 text-right font-bold c-ok">${formatEuro(a.ca)}</td></tr>`).join('')}</tbody></table></div>`:'';
    ...
    const pepOtherRows=pepOther.map(p=>{
      const ecartStr=p.ecartPct!==null?`<span class="text-orange-700 font-extrabold">+${p.ecartPct}%</span>`:'<span class="c-danger font-extrabold">Absent</span>';
      const myFreqStr=p.myFreq>0?`<span class="t-primary">${p.myFreq}</span>`:'<span class="text-red-500 font-semibold">0</span>';
      return`<tr class="border-b hover:i-caution-bg/40"><td class="py-1.5 px-3 font-mono t-tertiary whitespace-nowrap">${p.code}</td><td class="py-1.5 px-3 font-semibold t-primary">${p.lib}</td><td class="py-1.5 px-3 t-tertiary text-[11px]">${p.fam||'—'}</td><td class="py-1.5 px-3 text-center">${myFreqStr}</td><td class="py-1.5 px-3 text-center font-extrabold text-orange-700">${p.compFreq}</td><td class="py-1.5 px-3 text-center">${ecartStr}</td><td class="py-1.5 px-3 text-right t-secondary whitespace-nowrap">${p.caComp>0?formatEuro(p.caComp):'—'}</td></tr>`;
    }).join('');
  ...
  function renderObsArticleSearch(){
    ...
    res.innerHTML=rows.length?`<div class="bg-white border rounded-xl overflow-hidden shadow-sm mt-1">${rows.join('')}</div>`:'<p class="text-sm t-disabled mt-2 text-center py-2">Aucun article trouvé.</p>';
  }
  ...
  function renderDashboardAndCockpit(){
    ...
    p=[];Object.keys(byFamily).sort((a,b)=>byFamily[b]-byFamily[a]).slice(0,10).forEach(f=>{const fAttr=f.replace(/&/g,'&amp;').replace(/"/g,'&quot;');p.push(`<tr class="hover:s-card-alt"><td class="py-2 truncate max-w-[160px]" title="${fAttr}">${fAttr}</td><td class="py-2 text-right text-orange-600 font-bold">${formatEuro(byFamily[f])}</td><td class="py-2 text-center"><button class="diag-btn bg-blue-100 c-action" data-fam="${fAttr}" onclick="openDiagnostic(this.dataset.fam,'stock')">🔍</button></td></tr>`);});document.getElementById('dashFamilyTable').innerHTML=p.join('');
    ...
    (function(){
      ...
      if(sorted.length>1){const table=el.closest('table');if(table){let tf=table.querySelector('tfoot');if(!tf){tf=document.createElement('tfoot');table.appendChild(tf);}tf.innerHTML=`<tr><td colspan="4" class="py-1 px-2 text-right"><button onclick="event.stopPropagation();_copyAllCodesDirect(this,this.dataset.codes)" data-codes="${sorted.map(i=>i.code).join(',')}" class="text-[10px] t-disabled hover:t-primary bg-white border b-default rounded px-1.5 py-0.5">📋 Copier ${sorted.length} codes</button></td></tr>`;}}
    })();
  ...
  function renderTable(pageOnly){
    ...
    for(const r of pd){
      const bg=r.isNouveaute?'i-ok-bg':(r.nouveauMin>0?'bg-white':'s-card-alt t-disabled');
      const sc=r.stockActuel<0?'c-danger font-extrabold':'text-orange-600 font-bold';
      const br=getAgeBracket(r.ageJours);
      const _medMinCell=showMed?(r.medMinReseau!=null?`<td class="px-2 py-2 text-center text-xs ${r.nouveauMin>2*r.medMinReseau?'text-orange-700 i-caution-bg font-bold':r.nouveauMin>r.medMinReseau?'text-amber-600 font-semibold':'t-disabled'}" title="Méd. réseau MIN = ${Math.round(r.medMinReseau)}">${Math.round(r.medMinReseau)}</td>`:'<td class="px-2 py-2 text-center text-xs text-gray-300">—</td>'):'';
      const _medMaxCell=showMed?(r.medMaxReseau!=null?`<td class="px-2 py-2 text-center text-xs ${r.nouveauMax>2*r.medMaxReseau?'text-orange-700 i-caution-bg font-bold':r.nouveauMax>r.medMaxReseau?'text-amber-600 font-semibold':'t-disabled'}" title="Méd. réseau MAX = ${Math.round(r.medMaxReseau)}">${Math.round(r.medMaxReseau)}</td>`:'<td class="px-2 py-2 text-center text-xs text-gray-300">—</td>'):'';
    p.push(`<tr class="border-b hover:i-info-bg ${bg}">
      ...
      <td class="px-2 py-2 text-center font-extrabold text-xs ${r.abcClass==='A'?'text-green-700 i-ok-bg':r.abcClass==='B'?'c-action i-info-bg':r.abcClass==='C'?'text-orange-700 i-caution-bg':'t-disabled'}">${r.abcClass||'—'}</td>
      <td class="px-2 py-2 text-center font-extrabold text-xs ${r.fmrClass==='F'?'text-green-700 i-ok-bg':r.fmrClass==='M'?'c-action i-info-bg':r.fmrClass==='R'?'c-danger i-danger-bg':'t-disabled'}">${r.fmrClass||'—'}</td>
    </tr>`);}
  ...
  function renderABCTab(){
    ...
    html+='<th class="p-3 text-sm font-extrabold text-center text-green-700">F<br><span class="text-[10px] font-normal t-disabled">Fréquent ≥12</span></th>';
  ...
  function openArticlePanel(code,source){
    ...
    // Stock section
    const stockColor=r.stockActuel<=0?'text-red-400 font-extrabold':'c-ok';
    ...
    if(buyerList.length){
      ...
      const rows=buyerList.slice(0,5).map(b=>`<tr class="border-t border-slate-700"><td class="py-1 px-2 font-mono text-[10px] t-disabled">${b.cc}</td><td class="py-1 px-2 text-xs">${b.nom}${_unikLink(b.cc)}${b.statusBadge?' '+b.statusBadge:''}</td><td class="py-1 px-2 text-right text-xs font-bold ${b.caArt>0?'c-ok':'t-tertiary'}">${b.caArt>0?formatEuro(b.caArt):'—'}</td><td class="py-1 px-2 text-center text-[10px] ${b.daysSince!==null&&b.daysSince>30?'text-rose-400':'t-disabled'}">${b.daysSince!==null?b.daysSince+'j':'—'}</td></tr>`).join('');
    ...
    if(_S.storesIntersection.size>1&&_S.selectedMyStore){
      ...
      if(freqs.length>1){
        ...
        const rankCls=rank<=Math.ceil(nbAg/3)?'c-ok':rank<=Math.ceil(2*nbAg/3)?'c-caution':'text-red-400';
  ...
  function _renderDiagnosticCellPanel(key,cellArts){
    ...
    const nbAgences=_S.storesIntersection.size;
    const agenceCtxHtml=`<p class="text-xs text-cyan-300 font-semibold mt-1">🏪 ${agenceLabel}${hasMulti?` <span class="t-inverse-muted font-normal">(${nbAgences} agences chargées)</span>`:'<span class="t-inverse-muted font-normal"> (mono-agence)</span>'}</p>`;
    const filesHtml=[`<span class="c-ok">✅ Ventes</span>`,`<span class="c-ok">✅ Stock</span>`,hasMulti?`<span class="c-ok">✅ ${nbAgences} agences</span>`:`<span class="t-inverse-muted">❌ 1 agence</span>`,hasChal?`<span class="c-ok">✅ Chalandise</span>`:`<span class="t-inverse-muted">❌ Chalandise</span>`].join('<span class="text-slate-600 mx-1">·</span>');
    ...
    const titleHtml=`🔍 Diagnostic : <span class="text-cyan-400">${key}</span> — <span class="text-amber-300">${LABELS[key]||key}</span> <span class="t-inverse-muted font-normal text-sm">(${cellArts.length} articles)</span>`;
    panel.innerHTML=`
      <div class="flex items-start justify-between mb-5">
        <div>
  ...
  function _diagRenderV1(v,hasNetworkData){
    if(v.status==='absent')return`<div class="diag-voyant diag-v1"><div class="diag-voyant-hdr"><span class="font-extrabold text-sm text-blue-300">📦 Mon Agence</span>${_diagBadge('absent')}</div><p class="text-xs t-inverse-muted mt-1">Vous ne stockez aucun article dans cette famille.</p>${hasNetworkData?'<p class="text-[10px] c-ok mt-1">→ Consultez Le Réseau ci-dessous — d\'autres agences vendent dans cette famille.</p>':''}</div>`;
    const rupIcon=v.ruptures.length===0?'✅':v.ruptures.length<=3?'⚠️':'🚨';
    const rupClass=v.ruptures.length===0?'c-ok':v.ruptures.length<=3?'c-caution':'text-rose-400';
    ...
    const mmClass=v.nbMM===0?'c-ok':v.nbMM<=5?'c-caution':'text-rose-400';
    ...
    return`<div class="diag-voyant diag-v1">
      ...
      ${top5.length?`<div class="mb-2">${top5.map(r=>`<div class="flex items-start gap-2 py-1 px-2 mb-0.5 rounded s-panel-inner/60 text-[11px]"><span class="t-inverse-muted">·</span><span class="flex-1"><span class="font-mono t-inverse-muted">${r.code}</span> <span class="text-white font-semibold">${r.lib}</span> — Fréq ${r.W}, rupture ${r.jours}j → <span class="font-bold ${r.jours>=25?'c-caution':'text-cyan-400'}">${actionLabel(r)}</span>${r.ca>0?' <span class="text-rose-400 text-[10px]">('+formatEuro(r.ca)+')</span>':''}</span></div>`).join('')}${v.ruptures.length>5?`<p class="text-[10px] t-inverse-muted ml-4">… et ${v.ruptures.length-5} autre${v.ruptures.length-5>1?'s':''}</p>`:''}</div>`:''}
      <p class="text-xs ${mmClass} font-bold mb-1 mt-2">${mmIcon} ${mmText}</p>
      ${top5MM.length?`<div class="mb-1">${top5MM.map(r=>`<div class="flex items-start gap-2 py-0.5 px-2 mb-0.5 rounded s-panel-inner/60 text-[11px]"><span class="t-inverse-muted">·</span><span class="flex-1"><span class="font-mono t-inverse-muted">${r.code}</span> ${r.lib} — MIN <span class="c-caution">${r.ancienMin}</span> → <span class="c-ok font-bold">${r.nouveauMin}</span> <span class="text-rose-400 text-[10px]">(+${r.ecart})</span></span></div>`).join('')}${v.mmDetail.length>5?`<p class="text-[10px] t-inverse-muted ml-4">… et ${v.mmDetail.length-5} autre${v.mmDetail.length-5>1?'s':''}</p>`:''}</div>`:''}
      ${dormHtml}
    </div>`;
  }
  ...
  function _diagRenderL1(l){
    const verdictClass=l.ruptures.length===0?'c-ok':l.ruptures.length<=3?'c-caution':'text-rose-400';
  ...
  function _diagRenderL2(l,hasBench,refStore){
    ...
    const verdictClass=nbTotal===0?'c-ok':nbTotal<=5?'c-caution':'text-rose-400';
    ...
    return`<div class="diag-level">
      ...
      <p class="text-xs ${verdictClass} font-bold mb-2">${verdictIcon} ${verdictText}</p>
      ${top5.length?`<div class="mb-2"><p class="text-[10px] t-inverse-muted font-bold uppercase tracking-wide mb-1.5">🔧 Plus gros écarts à corriger :</p>${top5.map(r=>`<div class="flex items-start gap-2 py-1 px-2 mb-0.5 rounded s-panel-inner/60 text-[11px]"><span class="t-inverse-muted shrink-0">·</span><span class="flex-1"><span class="font-mono t-inverse-muted">${r.code}</span> <span class="text-white font-semibold">${r.lib}</span> — <span class="t-inverse">MIN actuel <span class="c-caution">${r.ancienMin}</span> → recommandé <span class="c-ok font-bold">${r.nouveauMin}</span></span> <span class="text-rose-400 text-[10px]">(+${r.ecart})</span>${hasBench&&r.refFreq>0?' <span class="t-inverse-muted text-[10px]">· réf. vend '+r.refFreq+'×</span>':''}</span></div>`).join('')}${l.detail.length>5?`<p class="text-[10px] t-inverse-muted mt-1 ml-4">… et ${l.detail.length-5} autre${l.detail.length-5>1?'s':''} à recalibrer</p>`:''}</div>`:''}
  ...
  function _diagRenderV2(v,hasChal){
    ...
    if(v.cellMode){
      const metiersHtml=v.metiers.map((m,mIdx)=>{
        ...
        return`<div class="${mIdx>0?'mt-3 pt-3 border-t border-slate-700':''} s-panel-inner/50 rounded-lg p-3">
          <div class="flex flex-wrap items-center gap-2 mb-1.5">
            <span class="text-xs font-extrabold text-rose-300">${m.metier}${_isMetierStrategique(m.metier)?' <span class="c-caution text-[10px]">⭐</span>':''}</span>
    ...
    const metiersHtml=v.metiers.map((m,mIdx)=>{
      ...
      const counters=[m.p5?`<span class="c-ok">${m.p5} actifs PDV (${pctActifPDV}%)</span>`:'',m.p1?`<span class="c-caution">${m.p1} actifs sans achat PDV</span>`:'`',(m.p2+m.p3)?`<span class="text-rose-400">${m.p2+m.p3} perdus</span>`:''].filter(s=>s&&s!=='`').join(' · ');
      const metierEsc=m.metier.replace(/'/g,"\\'").replace(/"/g,'&quot;');
      return`<div class="${mIdx>0?'mt-3 pt-3 border-t border-slate-700':''} s-panel-inner/50 rounded-lg p-3">
        <div class="flex flex-wrap items-center gap-2 mb-1.5">
          <span class="text-xs font-extrabold text-rose-300">${m.metier}${_isMetierStrategique(m.metier)?' <span class="c-caution text-[10px]">⭐</span>':''}</span>
    ...
    return`<div class="diag-voyant diag-v2">
      <div class="diag-voyant-hdr"><span class="font-extrabold text-sm text-purple-300">👥 Mes Clients</span>${_diagBadge(v.status)}</div>
      <p class="text-[10px] t-inverse-muted mb-3">Top ${v.metiers.length} métier${v.metiers.length>1?'s':''} acheteurs · <strong class="text-white">${v.totalBuyers}</strong> identifié${v.totalBuyers>1?'s':''} dans la chalandise${v.perdus>0?' · <span class="text-rose-400">'+v.perdus+' perdus</span>':''}</p>
      ${metiersHtml}
    </div>`;
  }
  ...
  function _diagRenderV3(v,hasMulti){
    ...
    const insnsRows=(v.inStockNotSold||[]).map(a=>`<tr class="border-t border-amber-900/30"><td class="py-1 px-2 font-mono t-inverse-muted">${a.code}</td><td class="py-1 px-2 max-w-[140px] truncate">${a.lib}</td><td class="py-1 px-2 text-center text-amber-300 font-bold">${a.stockActuel}</td><td class="py-1 px-2 text-center">${a.nbStores}/${v.nbOtherStores}</td><td class="py-1 px-2 text-center font-bold">${a.medFreq}</td></tr>`).join('');
    ...
    const ecartColor=v.caEcart>=0?'c-ok':v.caEcart>=-50?'c-caution':'text-red-400';
    ...
    const exclRows=(v.exclusives||[]).map(a=>`<tr class="border-t border-blue-900/30"><td class="py-1 px-2 font-mono t-inverse-muted">${a.code}</td><td class="py-1 px-2 max-w-[160px] truncate">${a.lib}</td><td class="py-1 px-2 text-center text-blue-300 font-bold">${a.myFreq}</td></tr>`).join('');
    return`<div class="diag-voyant diag-v3">
  ...
  function _diagRenderL3(l,hasBench,hasTerr){
    ...
    const rows=(l.missing||[]).map(a=>{
      const abcColor=a.abcClass==='A'?'c-ok':a.abcClass==='B'?'text-blue-400':'t-inverse-muted';
      const fmrColor=a.fmrClass==='F'?'c-ok':a.fmrClass==='M'?'text-blue-400':'text-red-400';
  ...
  function _diagRenderL1Metier(l){
    if(l.reason&&l.arts===0)return`<div class="diag-level"><div class="diag-level-hdr"><span class="font-bold text-sm text-blue-300">📦 Niveau 1 — Articles achetés</span>${_diagBadge('warn')}</div><p class="text-xs t-inverse-muted">${l.reason}</p></div>`;
    const verdictClass=l.ruptures.length===0?'c-ok':l.ruptures.length<=3?'c-caution':'text-rose-400';
    const verdictIcon=l.ruptures.length===0?'✅':l.ruptures.length<=3?'⚠️':'🚨';
  ...
  function _diagRenderL3Metier(l){
    ...
    const rows=(l.familles||[]).map(f=>{
      ...
      const textColor=pctCov>=70?'c-ok':pctCov>=40?'c-caution':'text-red-400';
    ...
    return`<div class="diag-level">
      ...
      <p class="text-xs t-inverse-muted mb-2">${l.totalArts} articles achetés · couverture rayon : <strong class="${l.pct>=70?'c-ok':l.pct>=40?'c-caution':'text-red-400'}">${l.pct}%</strong></p>
  ...
  function _diagRenderL4(l,hasChal){
    ...
    if(!l.metiers?.length)return`<div class="diag-level"><div class="diag-level-hdr"><span class="font-bold text-sm text-rose-300">👥 Niveau 3 — Clients métier</span>${_diagBadge('warn')}</div><p class="text-xs t-inverse-muted">⚠️ ${l.reason||'Aucun métier identifié dans la chalandise pour cette famille'}</p></div>`;
    const metiersHtml=l.metiers.map((m,mIdx)=>{
      const pctActifPDV=m.total>0?Math.round(m.p5/m.total*100):0;
      const counters=[m.p5?`<span class="c-ok font-bold">${m.p5} actifs PDV (${pctActifPDV}%)</span>`:'',m.p1?`<span class="c-caution">${m.p1} Legallais sans PDV</span>`:'',(m.p2+m.p3)?`<span class="text-rose-400">${m.p2+m.p3} perdus</span>`:''].filter(Boolean).join(' · ');
      const metierEsc=m.metier.replace(/'/g,"\\'").replace(/"/g,'&quot;');
      return`<div class="${mIdx>0?'mt-3 pt-3 border-t border-slate-700':''} s-panel-inner/50 rounded-lg p-3">
        <div class="flex flex-wrap items-center gap-2 mb-1.5">
          <span class="text-xs font-extrabold text-rose-300">${m.metier}${_isMetierStrategique(m.metier)?' <span class="c-caution text-[10px]" title="Métier stratégique Legallais">⭐</span>':''}</span>
          <span class="text-[10px] font-bold c-caution bg-amber-900/30 px-2 py-0.5 rounded-full">${m.pct}% des acheteurs</span>
        </div>
        <p class="text-[11px] t-inverse mb-1">→ <strong class="text-white">${m.total}</strong> clients dans votre zone → ${counters}</p>
        ${m.potentiel>0?`<p class="text-[11px] c-caution font-bold mb-1.5">→ Potentiel récupérable : ${formatEuro(m.potentiel)}</p>`:''}
    ...
    return`<div class="diag-level">
      ...
      ${metiersHtml}
      ${(l.crossCaptes||l.crossPot)?`<p class="text-[11px] t-inverse-muted mt-3 pt-2 border-t border-slate-700">🔀 Croisement zone : <span class="c-ok font-bold">🟢 ${l.crossCaptes||0} captés</span> · <span class="text-rose-400 font-bold">🔴 ${l.crossPot||0} potentiels non captés</span></p>`:''}
    </div>`;
  }
  ...
  function _diagRenderPlan(famille,actions){
    if(!actions.length)return`<div class="mt-4 p-4 s-panel-inner border border-slate-700 rounded-xl"><p class="text-xs c-ok">✅ Aucune action prioritaire — votre agence est bien calibrée pour cette famille.</p></div>`;
    // Conclusion pure (famille non pertinente / marginale sans action)
    if(actions.length===1&&actions[0].isInfo)return`<div class="mt-4 p-4 s-panel-inner border border-slate-700 rounded-xl flex items-start gap-2"><span class="text-lg flex-shrink-0">${actions[0].src||'ℹ️'}</span><p class="text-xs t-inverse">${actions[0].label}</p></div>`;
    const realActs=actions.filter(a=>!a.isInfo);
    _S._diagPlanCopyText=realActs.map((a,i)=>`${i+1}. [${a.src||'📦'}] ${a.label}`).join('\n');
    return`<div class="mt-4 p-4 s-panel-inner border border-slate-600 rounded-xl">
      <div class="flex items-center justify-between mb-3">
        <h3 class="font-extrabold text-sm text-white">📋 Plan d'action — <span class="text-cyan-400">${famille}</span></h3>
        <div class="flex items-center gap-2">
          <button id="_diagCopyBtn" onclick="_copyDiagPlan()" class="text-[10px] bg-slate-700 hover:bg-slate-600 t-inverse py-1 px-3 rounded font-bold">📋 Copier</button>
          <button onclick="exportDiagnosticCSV('${famille.replace(/'/g,"\\'")}')" class="text-[10px] bg-slate-700 hover:bg-slate-600 t-inverse py-1 px-3 rounded font-bold">📥 CSV</button>
        </div>
      </div>
      ${actions.map((a,i)=>a.isInfo?`<div class="px-3 py-2 rounded-lg bg-slate-700/50 border border-slate-600 flex items-center gap-2 text-xs t-inverse mt-1"><span class="flex-shrink-0">${a.src||'ℹ️'}</span><span>${a.label}</span></div>`:`<div class="diag-action-row" onclick="executeDiagAction(${i})"><div class="flex items-center gap-2 min-w-0"><span class="diag-src-tag flex-shrink-0">${a.src||'📦'}</span><span class="text-xs font-semibold truncate">${i+1}. ${a.label}</span></div><span class="t-inverse-muted text-[10px] ml-3 flex-shrink-0">→ Voir</span></div>`).join('')}
    </div>`;
  }
