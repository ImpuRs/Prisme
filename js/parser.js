...
function computeBenchmark() {
  ...
  for (const store of cs) { storesFamFreq[store] = {}; const sv = ventesParMagasin[store] || {}; for (const [code, data] of Object.entries(sv)) { if (!/^\d{6}$/.test(code)) continue; if (obsFilterUnivers && articleUnivers[code] !== obsFilterUnivers) continue; const fam = articleFamille[code] || ''; if (fam) storesFamFreq[store][fam] = (storesFamFreq[store][fam] || 0) + data.countBL; } }
  ...
  for (const store of allOtherStores) { storeFamCA[store] = {}; storeFamRef[store] = {}; let ca = 0; const sv = ventesParMagasin[store] || {}; for (const [code, data] of Object.entries(sv)) { if (!data.sumPrelevee && !data.sumEnleve) continue; const fam = articleFamille[code]; if (!fam) continue; if (!/^\d{6}$/.test(code)) continue; if (obsFilterUnivers && articleUnivers[code] !== obsFilterUnivers) continue; const lineCA = artCA(data); storeFamCA[store][fam] = (storeFamCA[store][fam] || 0) + lineCA; storeFamRef[store][fam] = (storeFamRef[store][fam] || 0) + 1; ca += lineCA; } storeTotalCA[store] = ca; }
  ...
  for (const [code, data] of Object.entries(myV)) { if (!data.sumPrelevee && !data.sumEnleve) continue; if (!/^\d{6}$/.test(code)) continue; const fam = articleFamille[code]; const lineCA = artCA(data); myTotalCA += lineCA; if (!fam) continue; myFamCA[fam] = (myFamCA[fam] || 0) + lineCA; myFamRef[fam] = (myFamRef[fam] || 0) + 1; }
