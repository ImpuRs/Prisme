...
export function computeBenchmark() {
  ...
  { const allSt=Object.keys(sp);const stCA={};let totCA=0;for(const s of allSt){const sv=_S.ventesParMagasin[s]||{};const ca=Object.values(sv).reduce((acc,v)=>acc+(v.sumCA||0),0);stCA[s]=ca;totCA+=ca;}for(const s of allSt)sp[s].pdmBassin=totCA>0?+(stCA[s]/totCA*100).toFixed(1):0; }
