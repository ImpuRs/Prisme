...
function computeBenchmark() {
  ...
  for (const store of allOtherStores) { storeFamCA[store] = {}; storeFamRef[store] = {}; let ca = 0; const sv = ventesParMagasin[store] || {}; for (const [code, data] of Object.entries(sv)) { if (!data.sumPrelevee && !data.sumEnleve) continue; const fam = articleFamille[code]; if (!fam) continue; if (!/^\d{6}$/.test(code)) continue; if (obsFilterUnivers && articleUnivers[code] !== obsFilterUnivers) continue; const prix = prixLookup[code] || 0; const lineCA = artCA(data, prix); storeFamCA[store][fam] = (storeFamCA[store][fam] || 0) + lineCA; storeFamRef[store][fam] = (storeFamRef[store][fam] || 0) + 1; ca += lineCA; } storeTotalCA[store] = ca; }
  ...
  for (const fam of allFams2) {
    ...
    if (compV) { for (const [code, data] of Object.entries(compV)) { if (articleFamille[code] !== fam || (data.sumPrelevee || 0) + (data.sumEnleve || 0) <= 0) continue; if ((myV[code]?.sumPrelevee || 0) > 0) continue; const r = finalDataByCode[code]; const statutMe = !r ? '❌ Absent' : r.stockActuel > 0 ? '✅ En stock' : r.ancienMax > 0 ? '⚠️ Rupture' : '❌ Absent'; const caA = artCA(data, prixLookup[code] || 0); if (!/^\d{6}$/.test(code)) { specialArts.push({ code, freqOther: data.countBL, caOther: Math.round(caA) }); continue; } const r2 = finalDataByCode[code]; const statMe2 = !r2 ? '❌ Absent' : r2.stockActuel > 0 ? '✅ En stock' : r2.ancienMax > 0 ? '⚠️ Rupture' : '❌ Absent'; missingArts.push({ code, lib: _splitLib(libelleLookup[code] || 'Libellé non disponible'), freqOther: data.countBL, caOther: Math.round(caA), statutMe: statMe2 }); }
